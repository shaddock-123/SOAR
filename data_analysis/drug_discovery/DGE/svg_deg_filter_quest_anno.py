...
dge_dir = '/projects/b1131/SpatialT/drug-target/'+dsid+'/'+sampleid+'/DGE_anno/'
svg_dir = '/projects/b1131/SpatialT/'+tech+'/'+pid+'/'+dsid+'/'+sampleid+'/analysis/SVG/'
sample_dge_dir = dge_dir.split('DGE_anno/')[0]

svg = pd.read_csv(svg_dir+'SpatialDE_results.tsv', sep='\t')
svg_filter = svg[svg['qval']<=0.1]
print('svg read in')

if not os.path.exists(sample_dge_dir+'DGE_anno_SVG'):
    os.makedirs(sample_dge_dir+'DGE_anno_SVG')
...
for file in os.listdir(dge_dir):
    ...
    deg.to_csv(sample_dge_dir+'DGE_anno_SVG'+'/'+cell_type+'.csv',index=False)   
